...
class NorthgateAdapter(BaseIntakeAdapter):
    """Northgate Regional Hospital 的 XML 接入。

    EHR 系统导出的 XML, 字段命名是 HL7 风格, 姓名用 ^ 分隔,
    日期是 YYYYMMDD, 剂量拆成 value+unit 属性, 诊断按 rank 排序。
    """

    source = "northgate"

    # ICD-10: 一个字母 + 两位数字, 可选小数点后再跟若干数字/字母 (如 G70.00、E11.9)。
    # 跟 ClinicBAdapter._ICD10 平行 (故意不抽到基类共享 —— 那是另一个重构话题)。
    _ICD10 = re.compile(r"^[A-Z][0-9]{2}(\.[0-9A-Z]+)?$")

    def parse(self) -> ET.Element:
        """把 self.raw (XML 文本 / 字节) 解析成 ElementTree 树根。

        只做"读取与结构化": 先防御性确认类型, 再交给 ET.fromstring。字段映射
        (^ 分隔的姓名、YYYYMMDD 日期、rank 排序…) 全留给 transform, 这里不碰。
        ET.fromstring 自己抛的 ParseError (XML 损坏、非法编码…) 直接冒出去, 不 wrap。
        """
        if not isinstance(self.raw, (str, bytes)):
            raise ValueError(
                "NorthgateAdapter expects str or bytes, got %s"
                % type(self.raw).__name__
            )
        return ET.fromstring(self.raw)

    def transform(self, parsed: ET.Element) -> InternalOrder:
        """把 XML 树根映射成嵌套 InternalOrder。只搬字段, 不做业务校验。

        必填字段直接 find()/.get() 取, 缺了让 AttributeError/KeyError 冒出去;
        选填字段用 .findtext()/.get() (找不到返回 None), 不丢信息也不报错。
        """
        # 1) 病人: MRN 是 Subject 的属性; Name 按 ^ 拆 (姓^名^中间名缩写);
        #    BirthDate 是 YYYYMMDD; Sex 选填。
        subject = parsed.find("Subject")
        last_name, first_name, *rest = subject.findtext("Name").split("^")
        patient = InternalPatient(
            first_name=first_name,
            last_name=last_name,
            mrn=subject.get("mrn"),
            dob=datetime.strptime(subject.findtext("BirthDate"), "%Y%m%d").date(),
            middle_initial=rest[0] if rest else None,
            gender=subject.findtext("Sex"),
        )

        # 2) 开方医生: NPI 是 OrderingClinician 的属性; 名字在 FullName。
        clinician = parsed.find("OrderingClinician")
        provider = InternalProvider(
            name=clinician.findtext("FullName"),
            npi=clinician.get("npi"),
        )

        # 3) 药品: dose 由 Strength 的 value + unit 拼成 "60 mg"; ndc / 频率选填。
        drug = parsed.find("Drug")
        strength = drug.find("Strength")
        medication = InternalMedication(
            name=drug.findtext("Label"),
            ndc=drug.findtext("NationalDrugCode"),
            dose=f'{strength.get("value")} {strength.get("unit")}',
            frequency=drug.findtext("Sig"),       # Northgate 叫 Sig, 内部统一叫 frequency
        )

        # 4) 诊断: 按 rank 升序, rank=1 是主诊断, 其余 (rank>=2) 进 additional。
        ranked = sorted(
            parsed.find("Conditions").findall("Condition"),
            key=lambda c: int(c.get("rank")),
        )
        diagnosis = InternalDiagnosis(
            primary=ranked[0].get("code"),
            additional=[c.get("code") for c in ranked[1:]],
        )

        # 5) 用药史 (所有 <Med>) 与临床记录 (ClinicalNarrative) 均选填。
        prior_meds = parsed.find("PriorMeds")
        medication_history = (
            [m.text for m in prior_meds.findall("Med")] if prior_meds is not None else []
        )
        ...
        return InternalOrder(
            ...
            patient_records=parsed.findtext("ClinicalNarrative", ""),
            source=self.source,
        )

    def validate(self, order: InternalOrder) -> None:
        """校验业务规则: NPI 十位、MRN 九位、ICD-10 格式 (primary + 每个 additional)。

        跟 ClinicBAdapter.validate 几乎一致, 唯一差异是 MRN 为 9 位 (Northgate 自家
        体系), NPI 仍是 10 位 (全国统一)。不合格抛 ValueError。
        """
        npi = order.provider.npi
        if not (isinstance(npi, str) and npi.isdigit() and len(npi) == 10):
            raise ValueError("invalid NPI %r: must be a 10-digit numeric string" % (npi,))
        ...
        mrn = order.patient.mrn
        if not (isinstance(mrn, str) and mrn.isdigit() and len(mrn) == 9):
            raise ValueError("invalid MRN %r: must be a 9-digit numeric string" % (mrn,))
...
_ADAPTERS: dict[str, type[BaseIntakeAdapter]] = {
    ...
    NorthgateAdapter.source: NorthgateAdapter,
    # 新来源在此登记, 例如:
